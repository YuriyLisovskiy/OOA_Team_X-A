...
class ReplyToCommentAPITestCase(APIFactoryTestCase):
	...
	def test_ReplyToComment(self):
		request = self.request_factory.post(reverse('api_v1:artwork:edit_comment', args=[16]), {'text': 'New text'})
	...
	def test_ReplyToCommentWithReplies(self):
		request = self.request_factory.post(reverse('api_v1:artwork:edit_comment', args=[1]), {'text': 'New text'})
		force_authenticate(request, user=self.user_olivia)
		old_count = len(CommentsAPIView.as_view()(self.request_factory.get(reverse('api_v1:artwork:get_comments', args=[1]), {'answers': True}), pk=1).data['results'])
		response = self.view(request, pk=1)
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
		self.assertEqual(len(CommentsAPIView.as_view()(self.request_factory.get(reverse('api_v1:artwork:get_comments', args=[1]), {'answers': True}), pk=1).data['results']), old_count+1)

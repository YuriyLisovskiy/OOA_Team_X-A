...
class EditCommentAPITestCase(APIFactoryTestCase):
	...
	def test_EditCommentUnauthenticated(self):
		request = self.request_factory.get(reverse('api_v1:artwork:get_comment', args=[16]), {'text': 'New text'})
		response = self.view(request, pk=14)
		self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

	def test_EditCommentWithReplies(self):
		request = self.request_factory.put(reverse('api_v1:artwork:edit_comment', args=[1]), {'text': 'New text'})
		force_authenticate(request, user=self.user_user)
		response = self.view(request, pk=1)
		self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

	def test_EditNonexistentComment(self):
		request = self.request_factory.put(reverse('api_v1:artwork:edit_comment', args=[9999]), {'text': 'New text'})
		force_authenticate(request, user=self.user_user)

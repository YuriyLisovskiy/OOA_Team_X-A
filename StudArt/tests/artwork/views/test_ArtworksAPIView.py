...
class ArtworksAPITestCase(APIFactoryTestCase):
	...
	def test_getArtworksFourColumnsFirstPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 4, 'page': 1})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data['count'], 12)
	...
	def test_getArtworksFourColumnsSecondPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 4, 'page': 2})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data['count'], 12)
		self.assertEqual(len(response.data['results']), 4)
	...
	def test_getArtworksOutOfBoundsPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 3, 'page': 5})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

	def test_getArtworksFilterByTagFirstPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 3, 'page': 1, 'tag': 'tag1'})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_getArtworksFilterByTagSecondPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 3, 'page': 2, 'tag': 'tag1'})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data['count'], 5)
		self.assertEqual(len(response.data['results']), 2)
	...
	def test_getArtworksFilterByAuthorFirstPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 3, 'page': 1, 'author': 'User'})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_getArtworksFilterByAuthorSecondPage(self):
		request = self.request_factory.get('/api/v1/artworks', {'columns': 3, 'page': 2, 'author': 'User'})
		response = self.view(request)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data['count'], 6)
		self.assertEqual(len(response.data['results']), 3)
